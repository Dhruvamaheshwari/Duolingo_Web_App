    ...
    <div className="flex min-h-screen flex-col bg-[#f7f9fa] font-sans text-gray-800">
      ...
      <header className="sticky top-0 z-10 flex h-16 w-full items-center justify-between border-b-2 border-gray-200 bg-white px-4 md:px-8">
        ...
        <div className="flex items-center gap-6 font-bold text-gray-500">
          ...
          <span className="text-gray-800 border-b-2 border-gray-800 pb-1">LEADERBOARD</span>
      ...
      <main className="mx-auto flex w-full max-w-2xl flex-col items-center py-10 px-4">
        ...
        <div className="w-full mb-8 text-center flex flex-col items-center">
          ...
          <h1 className="text-3xl font-extrabold text-gray-800">League Standings</h1>
          <p className="text-gray-500 font-bold mt-2">Compete with other learners by earning XP.</p>
        </div>

        {/* Leaderboard List */}
        <div className="w-full rounded-2xl border-2 border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col">
          {leaderboard.map((entry, idx) => (
            <div 
              key={idx}
              className={`flex items-center justify-between p-4 px-6 border-b-2 border-gray-100 last:border-b-0 ${entry.is_current ? 'bg-blue-50' : 'bg-white'}`}
            >
              <div className="flex items-center gap-4">
                <span className={`font-bold w-6 text-center ${
                  entry.rank === 1 ? 'text-yellow-500 text-xl' : 
                  entry.rank === 2 ? 'text-gray-400 text-xl' : 
                  entry.rank === 3 ? 'text-orange-400 text-xl' : 'text-gray-500'
                }`}>
                  {entry.rank}
                </span>
                ...
                <span className={`font-bold ${entry.is_current ? 'text-blue-600' : 'text-gray-700'}`}>
